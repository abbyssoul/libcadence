...
*/
/*******************************************************************************
 * libcadence: Serial Port object
 *	@file		cadence/io/serial.hpp
 ******************************************************************************/
#pragma once
#ifndef CADENCE_IO_SERIAL_HPP
#define CADENCE_IO_SERIAL_HPP
...
struct SerialPortInfo {
    ...
    Solace::String description;

    /*! Hardware ID (e.g. VID:PID of USB serial devices) or "" (empty string) if not available. */
    Solace::String hardwareId;

    SerialPortInfo(Solace::Path&& path, Solace::String&& desc, Solace::String&& hwId)
        : file(std::move(path))
...
public:
     ...
     */
    static Solace::Array<SerialPortInfo> enumeratePorts();
...
public:
     ...
     */
    enum class Bytesize : Solace::uint8 {
        fivebits = 5,
        sixbits = 6,
        sevenbits = 7,
        eightbits = 8
     ...
     */
    enum class Parity : Solace::uint8 {
        none = 0,
        odd = 1,
        even = 2,
        mark = 3,
     ...
     */
    enum class Stopbits : Solace::uint8 {
        one = 1,
        two = 2,
        one_point_five
    };
     ...
     */
    enum class Flowcontrol: Solace::uint8 {
        none = 0,
        software,
        hardware
    };
...
public:
    ...
    * (due to timeout or select interruption). */
    bool waitReadable(Solace::uint32 timeout);

};

}  // namespace cadence
#endif  // CADENCE_IO_SERIAL_HPP
